...
function initGraph() {
  ...
  var link = svg.selectAll('path.link').data(currentGraph.edges)
  ...
  var linktext = svg.selectAll("g.linklabelholder").data(currentGraph.edges, function(d) { return d.source.id + d.target.id + d.label});
  ...
  var node = svg.selectAll("g.node").data(currentGraph.nodes)
  ...
  anchorNode.enter().append("svg:g").attr("class", "anchorNode");
  anchorNode.selectAll("*").remove(); // TODO: This is really not great...you want to update what's there, not remove and re-add
